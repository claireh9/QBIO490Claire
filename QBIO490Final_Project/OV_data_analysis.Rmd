...
```{r}
clinic$grade_binary<-ifelse(clinic$tumor_grade=="G3","High Grade","Low Grade")
```
...
```{r}
new_rna_clinical <- merge(clean_rna_clinical, 
                          clinic[,c("Tumor_Sample_Barcode","survival_time")],
                          by.x = "patient", by.y = "Tumor_Sample_Barcode",
                          all.x = TRUE)
```
...
```{r}
library(ggplot2)
maf_object@clinical.data$grade_bin<-ifelse(maf_object@clinical.data$tumor_grade=="G3","High Grade","Low Grade")
...
methylation_clinical$grade_bin <- ifelse(methylation_clinical$tumor_grade=="G3","High Grade","Low Grade")
