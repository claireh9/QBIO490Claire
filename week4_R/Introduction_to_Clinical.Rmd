...
mtcars$carb #Question: is carb short for carburetor
```
...
```{r}
write.csv(clinic, "/home1/heclaire/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```
...
$gender is a categorical variable (chr).  
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)?
This patient is 50 years old when diagnosed. 
...
The patient is 65 years old at their last follow up. 

```{r Exercise 3.1}
head(clinic)
str(clinic)

clinic[5,21] #patient 5 
age<-clinic[1,21]#patient 1's age 

age+as.numeric(clinic$last_contact_days_to[1])/365
...
```{r Exercise 4.1}
plot(clinic$age_at_diagnosis,clinic$lymph_nodes_examined_count,xlab="Age at Diagnosis",ylab="Lymph Nodes Examined",main="Age at Diagnosis vs. Lymph Nodes Examined",col="blue")
```
...
```{r Exercise 4.2}
boxplot(as.numeric(clinic$age_at_diagnosis)~clinic$race,xlab="Race",ylab="Age at Diagnosis",main="Age at Diagnosis by Race")
```
...
#OR
under30_over60_mask2<-ifelse(clinic$age_at_diagnosis<30,TRUE,ifelse(clinic$age_at_diagnosis>60,TRUE,FALSE))
```
...
#OR 
no_middle_clinic2<-clinic[under30_over60_mask2,]
```
...
The y axis represent survival probability. 
...
Age category young appears to have the greatest rate of survival. 
# 4. What age group appears to have the worst rate of survival? 
Age category old appears to have the worst rate of survival. 
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
The approximate 15 year survival rate for "Old" breast cancer patients is around 0.05, which is really close to 0.  
...
The p value is 0.016. This p value is very small and statistically significant. This suggests that age has a significant impact on breast cancer survival, with younger patients having a higher probability of survival compared to older patients.

```{r}
365*15 #number of days for 15 years 
```
